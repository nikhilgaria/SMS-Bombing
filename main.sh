...
banner() {
    ...
    if ! [ -x "$(command -v figlet)" ]; then
        echo 'Introducing X Project'
    else
        figlet EdWin
    fi
    if ! [ -x "$(command -v toilet)" ]; then
        echo -e "\e[4;34m This Bomber Was Created By \e[1;32mEdWin \e[0m"
    else
...
if [ -f .update ];then
    ...
    echo .
    install_deps
    echo This Script Was Made By Nikhil > .update
...
do
    ...
    if [ $ch -eq 1 ];then
        ...
        echo -e "\e[4;32m please enter valid input !!! \e[0m"
        pause
    fi
done
